...
**RPM** used cpio as its payload format from 1997 until RPM v6 shipped in September 2025. The cpio format has a 4GB file size limit baked into its header fields. For 28 years, no RPM package could contain a file larger than 4GB. RPM v6 finally dropped cpio in favor of a custom format.
...
The transition was rough in the ways you'd expect. CI pipelines across the industry broke because GitHub Actions' `macos-latest` runner was pinned to Homebrew 3.0.10, which still pointed at Bintray. During a [brownout on April 26](https://discuss.circleci.com/t/macos-image-users-homebrew-brownout-2021-04-26/39872), any system running an older Homebrew got 502 errors. Older bottle versions were never migrated, so anyone pinned to an old formula version got 404s and had to build from source. The fix was `brew update`, but CI environments cached old Homebrew versions and didn't auto-update.
...
The community-funded registries are where this adds up. rubygems.org, crates.io, PyPI, and hex.pm run on bandwidth donated by CDN providers, primarily Fastly, and packagist.org operates on donations and sponsorships. These registries serve terabytes of package data to millions of developers on infrastructure that someone is volunteering to cover.
...
**Registry UI confusion.** When a registry contains both container images and packages, the user experience gets muddled. GitHub Container Registry shows `docker pull` commands for everything, but a Homebrew bottle needs `brew install` and a Helm chart needs `helm pull`. Dan Lorenc, who worked on the OCI artifact spec, described the UX as "not great."

**Not all registries are equal.** The OCI 1.1 features that make non-container artifacts work well (custom `artifactType`, the referrers API, the `subject` field) aren't universally supported. The OCI Image Specification itself recommends avoiding artifacts "if you are concerned about portability." Registry implementations lag the spec, and the gap between what the spec allows and what any given registry supports is a source of bugs.
...
**Who pays.** GHCR storage and bandwidth are [currently free](https://docs.github.com/en/billing/concepts/product-billing/github-packages) for public images, with a promise of 30 days notice before that changes. At standard GitHub Packages rates ($0.25/GB/month for storage, $0.50/GB for bandwidth), Homebrew's bottle archive would cost substantially more than zero. GitHub absorbs that as an in-kind subsidy, and the Homebrew 3.1.0 release notes explicitly thank them for it.
...
Most registries were designed when packages had dozens of versions, not thousands, and pagination wasn't an obvious concern. PyPI's Simple Repository API lists all files for a package in one response, though PEP 700 added optional version filtering after the fact. crates.io takes a different approach with a git-based index that stores one file per crate, all versions as line-delimited JSON, while RubyGems' compact index and Go's module proxy both return complete version lists in a single response. None of these designed for pagination early on because the scale wasn't there yet, and retrofitting pagination onto an existing API is harder than building it in from the start.
