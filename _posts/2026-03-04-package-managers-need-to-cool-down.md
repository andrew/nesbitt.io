...
[Renovate](https://docs.renovatebot.com/key-concepts/minimum-release-age/) has had `minimumReleaseAge` (originally called `stabilityDays`) for years, adding a "pending" status check to update branches until the configured time has passed. [Depfu](https://depfu.com/blog/reasonably-up-to-date) shipped a "reasonably up-to-date" strategy in June 2019 that holds new releases for anywhere from a few days to a month before opening a PR, pitched at the time as a way to cut PR volume rather than a supply chain defence. [Mend Renovate 42](https://www.mend.io/blog/secure-npm-ecosystem-with-mend-renovate/) made a 3-day minimum release age the default for npm packages in their "best practices" config via the `security:minimumReleaseAgeNpm` preset, making cooldowns opt-out rather than opt-in for their users.
