...
[Renovate](https://docs.renovatebot.com/key-concepts/minimum-release-age/) has had `minimumReleaseAge` (originally called `stabilityDays`) for years, long before anyone else picked it up, adding a "pending" status check to update branches until the configured time has passed. [Mend Renovate 42](https://www.mend.io/blog/secure-npm-ecosystem-with-mend-renovate/) made a 3-day minimum release age the default for npm packages in their "best practices" config via the `security:minimumReleaseAgeNpm` preset, making cooldowns opt-out rather than opt-in for their users.
