...
**A license detector:** parses license files and metadata to display licensing information on package pages, with some registries flagging packages with missing or incompatible licenses, or enforcing policies about what can be published.
...
**A typosquatting detector:** scans for package names that look like misspellings of popular packages, which attackers register to catch developers who mistype an install command, and tries to detect and block them before they cause harm.
...
**An access control system:** determines who can publish or modify which packages through maintainer lists, organization teams, and role-based permissions, with some registries supporting granular controls like publish-only tokens or requiring multiple maintainers to sign off on releases.
...
**A security advisory database:** a catalog of known vulnerabilities mapped to affected package versions, so when a CVE is published the registry tracks which packages and version ranges are affected and tools can warn users.
...
**A malware scanner:** analyzes uploaded packages for malicious code before or after they're published, where automated static analysis catches obvious patterns but sophisticated attacks often require human review.

**A signing system:** cryptographic signatures that verify who published a package and that it hasn't been tampered with, and build provenance attestations that can prove the package was built from specific source code in a specific environment.

**Integrity verification:** checksums that detect corrupted or tampered downloads independent of signatures, so even without cryptographic verification you know you got what the registry sent.

**A security team:** people who triage vulnerability reports, investigate suspicious packages, coordinate takedowns, and respond to incidents, because automation helps but humans make the judgment calls.
...
**Trusted publishing:** allows CI systems to publish packages using short-lived OIDC tokens instead of long-lived secrets, so you don't have to store credentials in your build environment and compromised tokens expire quickly.

**An audit log:** records who published what package, when, from what IP address, and using what credentials, useful for forensics after a compromise or just understanding how a package evolved.
...
**A sustainability model:** how the registry, infrastructure, and maintainers are funded, whether through sponsorship programs, foundation grants, corporate backing, or just volunteer labor, and the answer varies widely and often determines what's possible.

So what is a package manager? It depends how far you zoom out. At the surface, it's a command that installs libraries. One level down, it's a dependency resolver and a reproducibility tool. Further still, it's a publishing platform, a search engine, a security operation, and part of global infrastructure. Most developers only interact with the first few items on this list.
