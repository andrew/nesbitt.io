...
A [2022 Checkmarx study](https://checkmarx.com/blog/a-new-type-of-supply-chain-attack-could-put-popular-admin-tools-at-risk/) found thousands of packages across Go and Swift vulnerable to repo-jacking, where an attacker registers an abandoned GitHub username and recreates a repo that existing packages still point to. Go's proxy mitigates this because cached modules don't re-fetch from the source, but SwiftPM has no such layer.
