...
A related but distinct cluster is compiler flag injection: a package smuggling `-fplugin=` or arbitrary `LDFLAGS` through to the C compiler at build time. This is mostly a Go-ecosystem pattern ([CVE-2018-6574](https://nvd.nist.gov/vuln/detail/CVE-2018-6574), [CVE-2023-29404](https://nvd.nist.gov/vuln/detail/CVE-2023-29404), [CVE-2023-39323](https://nvd.nist.gov/vuln/detail/CVE-2023-39323)) because of how cgo works, but the shape is the same: untrusted strings reaching a subprocess that treats some strings as instructions.

### Integrity checks that fail open

CWE-345 (insufficient verification of data authenticity) and CWE-347 (improper verification of cryptographic signature). A signature file is missing so verification is skipped. An error from the signature checker is treated as success. The hash is checked when the file enters the cache but not when it leaves. Dependency verification is enabled but a particular code path bypasses it.
...
[CVE-2019-3881](https://github.com/advisories/GHSA-g98m-96g9-wfjq), [CVE-2021-29428](https://github.com/gradle/gradle/security/advisories/GHSA-89qm-pxvm-p336), and [CVE-2023-38497](https://github.com/rust-lang/cargo/security/advisories/GHSA-j3xp-wfr4-hx87) are language-ecosystem examples. The system package managers have a much longer history here because they run as root: a symlink or hardlink swap between "installer creates the file" and "installer sets its permissions" gives privilege escalation, and the same tool can accumulate several of these over a decade as each fix turns out to be incomplete ([CVE-2017-7500](https://nvd.nist.gov/vuln/detail/CVE-2017-7500), [CVE-2021-35937](https://nvd.nist.gov/vuln/detail/CVE-2021-35937), [CVE-2021-35939](https://nvd.nist.gov/vuln/detail/CVE-2021-35939)).
...
This category is small in count but high in severity, because it usually means code execution from metadata alone, before the user has agreed to install anything. Use the safe-load variant of whatever parser you have and treat manifests as data, not programs.

### Resource exhaustion from crafted packages

CWE-400 (uncontrolled resource consumption) and CWE-1333 (inefficient regular expression complexity). A compressed archive that expands to fill the disk ([CVE-2022-36114](https://github.com/rust-lang/cargo/security/advisories/GHSA-2hvr-h6gw-qrxp)). A version string or git URL that hits exponential backtracking in a regex ([CVE-2013-4287](https://github.com/advisories/GHSA-9j7m-rjqx-48vh), [CVE-2025-8262](https://github.com/advisories/GHSA-9xhp-f235-5v37)). A JSON Schema with a `$ref` chain or nesting depth that exhausts memory or stack ([CVE-2025-55199](https://github.com/helm/helm/security/advisories/GHSA-9h84-qmv7-982p)). A tar header with a negative size that loops forever ([CVE-2018-1000075](https://github.com/advisories/GHSA-74pv-v9gh-h25p)).

Mostly a nuisance on a developer machine, more interesting on a shared CI runner or anything that processes packages automatically. Cap extracted size, cap recursion depth, and lint your regexes.
...
CWE-285 (improper authorisation) and CWE-863 (incorrect authorisation). The registry-side bug that matters most, because it turns directly into a supply-chain compromise without anyone's account being touched. A name-parsing edge case lets you yank and replace any package whose name contains a dash ([CVE-2022-29176](https://github.com/rubygems/rubygems.org/security/advisories/GHSA-hccv-rwq6-vh79)). An upload-ordering bug lets you overwrite the platform-specific variant of someone else's release in the CDN ([CVE-2022-29218](https://github.com/rubygems/rubygems.org/security/advisories/GHSA-2jmx-8mh8-pm8w)). A migration leaves an orphaned-ownership API in place for a decade and anyone can claim 1,800 packages ([CVE-2024-38368](https://github.com/CocoaPods/CocoaPods/security/advisories/GHSA-j483-qm5c-7hqx)). A microservice authorisation gap lets anyone publish a new version of any package, [reported](https://github.blog/security/supply-chain-security/githubs-commitment-to-npm-ecosystem-security/) and fixed without a CVE.
...
The actual code bugs in this category are about token hygiene: tokens generated with a non-cryptographic RNG and stored unhashed ([2020 crates.io advisory](https://blog.rust-lang.org/2020/07/14/crates-io-security-advisory.html)), an `X-Forwarded-For: 127.0.0.1` header bypassing a localhost-only admin account check ([CVE-2019-9733](https://nvd.nist.gov/vuln/detail/CVE-2019-9733)), a session-validation link that an attacker can get clicked zero-touch by the victim's mail scanner ([CVE-2024-38367](https://github.com/CocoaPods/CocoaPods/security/advisories/GHSA-52gf-m7v9-m333)).
