...
The package manager works. You can declare dependencies, fetch them, build against them. That's the easy part. The hard part is everything else: the ecosystem of tools, services, and infrastructure that makes a package manager usable in production.

Look at [the package management landscape](/2026/01/03/the-package-management-landscape.html). Dozens of categories, hundreds of tools. For Zig to have the same tooling support as npm or Cargo, each of those tools either needs to add Zig support, or the Zig community needs to build alternatives. That's a lot of work.
...
**Vulnerability databases.** The [GitHub Advisory Database](https://github.com/advisories) and [OSV](https://osv.dev) need advisories filed against Zig packages using Zig's identifier scheme. That requires agreeing on how to identify Zig packages (PURL has a [zig type proposal](https://github.com/package-url/purl-spec/issues/297) but it's not merged).
...
Beyond the engineering, there's human coordination. Shepherding PRs through repos maintained by volunteers with different priorities. Getting PURL proposals reviewed by a committee that meets sporadically. Convincing SCA vendors to prioritize your ecosystem over the next one in line. Zig's PURL proposal has been open since 2023. That's not a technical problem. It's part of why [package management is a wicked problem](/2026/01/23/package-management-is-a-wicked-problem.html): too many stakeholders, no single authority, solutions that create new problems.
