...
But ignoring those problems doesn't solve them. It just moves them elsewhere. The generated code still has to implement TLS, parse JSON, handle Unicode. The complexity doesn't vanish because you stopped calling it a dependency. Let's humor it though. I love going down a rabbit hole.
...
There's also the governance layer. Package registries aren't just file hosts. They decide who owns names, how disputes resolve, what gets removed, and how compromised accounts are handled. They're [governance providers](/2025/12/22/package-registries-are-governance-as-a-service.html), making judgment calls that keep ecosystems healthy: removing malware, transferring abandoned packages, enforcing naming policies. When npm restored left-pad after Azer Koçulu unpublished it, that was a governance decision that kept thousands of builds from breaking. A prompt registry has none of this. Who decides that a prompt is malicious? Who resolves conflicts when two prompt authors claim the same specification name? Who steps in when a widely-used prompt starts producing vulnerable code? These are governance questions, and they need institutions to answer them.

Yes, we've automated a lot of this collaboration. Maybe too much, if you've ever looked inside a node_modules folder. But underneath the automation there's still a core human process of collaboration and coordination happening. People building on each other's work without needing to talk to each other. That doesn't come from a prompt.

AI agents aren't going to stop using packages. Packages feature heavily in the training data, and every coding agent already reaches for `npm install` or `pip install` as a first instinct. The interesting work is happening in the opposite direction: making agents better at working *with* package managers rather than replacing them. I've been building [a skill that makes coding agents evaluate packages skeptically](/2026/01/21/an-ai-skill-for-skeptical-dependency-management.html) before suggesting them, checking that they exist, that they're maintained, that the standard library doesn't already cover the use case. And there's a broader opportunity to give agents a [shared protocol for package management](/2026/01/22/a-protocol-for-package-management.html), a common vocabulary for resolution, publishing and governance that works across ecosystems. AI that understands package managers deeply is far more useful than AI that tries to make them disappear.

[Package management is a wicked problem](/2026/01/23/package-management-is-a-wicked-problem.html). But the future isn't fewer packages. It's agents that understand them.
